...
import { ApplicationDto, BrokerApplicationPostResponseDto, BrokerApplicationsListBadRequestResponseDto, BrokerApplicationsListRequestDto, BrokerApplicationsListResponseDto } from './list-applications.dto';
...
export class BrokerApplicationsListController {
  ...
  @ApiOperation({
    ...
    description: 'Fetches the applications that the broker has submitted. The applications are optionally filtered by date and by task status.',
  })
  @ApiOkResponse({
  ...
  async post(
   ...
   let checkAmount:boolean;
    if(avgLoanAmount >= body.loanAmount){
        checkAmount = true;
    }
    else{
      checkAmount = false;
