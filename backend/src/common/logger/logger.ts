...
export class CustomLogger extends ConsoleLogger {
  ...
  log(message: string, ...rest): void {
    ...
    message = `\x1b[34m[LOG]\x1b[37m[${timestamp()}]\x1b[34m[${typeof message === 'object' ? JSON.stringify(message) : message
      }]`;
    if (stackFile) {
  ...
  warn(message: string, ...rest): void {
    ...
    message = `\x1b[33m[WARN]\x1b[36m[${timestamp()}]\x1b[37m[${typeof message === 'object' ? JSON.stringify(message) : message
      }]`;
    const stackFile = rest.pop();
    if (stackFile) {
      message += `\x1b[33m[${stackFile}]`;
